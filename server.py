...
def login_user():
	...
	return response("success", "Login ok", {"username": user["username"], "role": user["role"]})
...
def create_borrow_request():
	...
	if not book_id:
		return response("error", "book_id required", http_code=400)
	with closing(sqlite3.connect(DB_PATH)) as conn:
		...
		conn.commit()
	return response("success", "Borrow request created", None, 201)
