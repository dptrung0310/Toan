...
export const api = {
  ...
  createBook: (creds, title, author, description, url_image, quantity) =>
    callApi('/books', 'POST', { ...creds, title, author, description, url_image, quantity }),
  
  updateBook: (creds, id, fields) =>
    callApi(`/books/${id}`, 'PATCH', { ...creds, ...fields }),
  
  deleteBook: (creds, id) =>
    callApi(`/books/${id}`, 'DELETE', creds),
  ...
  listBorrowRequests: (status = '') =>
    callApi(status ? `/borrow-requests?status=${encodeURIComponent(status)}` : '/borrow-requests'),
